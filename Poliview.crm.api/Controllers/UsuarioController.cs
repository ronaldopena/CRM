    ...
    public class UsuarioController : ControllerBase
    {
        ...
        public IActionResult Authenticate(LoginRequisicao obj)
        {
            try
            {
                ...
                var errovalidacao = ValidaBodyLogin(obj);

                if (errovalidacao != "OK")
                {
                    var msgError = new HttpErrorMessage(400, errovalidacao);
        ...
        {
            var errovalidacao = ValidaBodyPrimeiroAcesso(obj);
        ...
        {
            var parametro = ParametrosService.consultar(connectionString);
        ...
        private Jwt CreateJwt()
        {
            ...
            jwt.key = "poliview.tecnologia.crm@2022.chave.segura.jwt.256bits";
            // var key = Convert.ToBase64String(RandomNumberGenerator.GetBytes(32));
            return jwt;
        }

        private string ValidaBodyLogin(LoginRequisicao obj)
        {
        ...
        private string ValidaBodyPrimeiroAcesso(PrimeiroAcessoRequisicao obj)
        {
            if (obj.idempresa == 0)
            {
                return "idempresa não informado";
            }
            if (string.IsNullOrEmpty(obj.senha))
            {
                return "Senha não informada";
