...
namespace Poliview.crm.service.email.Services
{
    public class ReceiveEmailGmailService : IReceiveEmailService
    {
        ...
        public ReceiveEmailGmailService(IConfiguration configuration, LogService logService)
        {
        ...
        private async Task ReceberEmailGmail(Serilog.ILogger log, ContaEmail conta)
        {
            ...
            {
                log.Error($"{conta.descricaoConta} - ERRO DE CONEXÃO COM O GMAIL ReceiveEmailService.cs(461) " + conta.descricaoConta);
        ...
        private static async Task DeletarEmailCaixaEntradaAsync(GmailService service, string UserId, string MessageId, Serilog.ILogger log)
        {
            try
            {
